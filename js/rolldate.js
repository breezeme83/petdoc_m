(function (global, factory) {
    ...
    var css = "ul{margin:0;padding:0}li{list-style-type:none}.rolldate-container{font-size:20px;color:#4e5263;text-align:center;font-family:;font-family:'Noto Sans', 'Apple SD Gothic Neo', 'Malgun Gothic', Dotum;}.rolldate-container header{position:relative;line-height:60px;font-size:18px;border-bottom:1px solid #e0e0e0}.rolldate-container .rolldate-mask{position:fixed;width:100%;height:100%;top:0;left:0;z-index:999;background-color:rgba(0,0,0,.6)}.rolldate-container .rolldate-panel{position:fixed;bottom:0;left:0;width:93.34%;margin:3.33%;height:246px;z-index:1000;background:#fff;-webkit-transition:all .3s ease-in-out;-o-transition:all .3s ease-in-out;transition:all .3s ease-in-out;-webkit-transform:translate3d(0,273px,0);transform:translate3d(0,273px,0)}.rolldate-container .rolldate-btn{width:100%;height:100%;color:#ff5a5c;font-size:17px;font-weight:bold;cursor:pointer;-webkit-tap-highlight-color:transparent}.rolldate-container.wx .rolldate-btn{height:150%}.rolldate-container .rolldate-confirm{display:inline-block;margin-top:16px;color:#ff5a5c;}.rolldate-container .rolldate-content{position:relative;top:20px}.rolldate-container .rolldate-wrapper{display:-webkit-box;display:-webkit-flex;display:-ms-flexbox;display:flex}.rolldate-container .rolldate-wrapper>div{-webkit-box-flex:1;-webkit-flex:1;-ms-flex:1;flex:1;height:173px;line-height:36px;overflow:hidden;-webkit-flex-basis:-8e;-ms-flex-preferred-size:-8e;flex-basis:-8e;width:1%}.rolldate-container .rolldate-wrapper ul{margin-top:68px}.rolldate-container .rolldate-wrapper li{height:36px}.rolldate-container .rolldate-dim{position:absolute;left:0;top:0;width:100%;height:68px;background:-o-linear-gradient(bottom,hsla(0,0%,100%,.4),hsla(0,0%,100%,.8));background:-webkit-gradient(linear, left bottom, left top, from(hsla(0, 0%, 100%, 0.4)), to(hsla(0, 0%, 100%, 0.8)));background:-o-linear-gradient(bottom, hsla(0, 0%, 100%, 0.4), hsla(0, 0%, 100%, 0.8));background:linear-gradient(0deg,hsla(0,0%,100%,.4),hsla(0,0%,100%,.8));pointer-events:none;-webkit-transform:translateZ(0);transform:translateZ(0);z-index:10}.rolldate-container .mask-top{border-bottom:1px solid #ebebeb}.rolldate-container .mask-bottom{top:auto;bottom:1px;border-top:1px solid #ebebeb}.rolldate-container .fadeIn{margin:3.3%;border-radius:8px;-webkit-transform:translateZ(0);transform:translateZ(0)}.rolldate-container .fadeOut{-webkit-transform:translate3d(0,273px,0);transform:translate3d(0,273px,0)}.bottom-block{position:absolute;bottom:0;height:56px;width:100%;border-top:1px solid #e1e4e6;}@media screen and (max-width:414px){.rolldate-container{font-size:20px}}@media screen and (max-width:320px){.rolldate-container{font-size:15px}}";
    styleInject(css);
    ...
    Rolldate.prototype = {
        ...
        createUI: function createUI() {
            var _this = this,
                ...
                config = _this.config,
                domId = data.domId,
                FormatArr = config.format.replace(/년|월|일/g, '').split(/-|\/|\s|:/g),
    ...
    function RollSelect() {
        ...
        if (config.value) {
            if (config.el) {
                if (el.nodeName.toLowerCase() == 'input') {
                    el.value = config.value;
                    el.bindValue = config.value;
                } else {
                    el.innerText = config.value;
                    _this.bindValue = config.value;
                }
            }
        }
    }

    RollSelect.prototype = {
        constructor: RollSelect,
        baseData: function baseData() {
            return {
                opts: {
                    el: '',
        ...
        createUI: function createUI() {
            ...
            var initialValue = _this.$(config.el).bindValue;
            
  
            for (var i = 0; i < len; i++) {
                var domMndex = 0;
  
                ul += '<div id="rollselect_' + i + '"><ul class="wheel-scroll">';
                var items = config.items[i];
                if(initialValue) {
                    var values = _this.$(config.el).bindValue.split(' ');        
                    for (var j = 0; j < items.length; j++) {
                        itemClass = items[j] == values[i] ? 'active' : '';
                        ul += '<li class="wheel-item ' + itemClass + '" data-index="' + domMndex + '">' + items[j] +'</li>';
                        domMndex++;
                    }
                } else {
                    for (var j = 0; j < items.length; j++) {
            ...
            var _loop = function _loop(_i) {
                var $id = `rollselect_${_i}`;
  
                _this.scroll[$id] = new bscroll_min('#' + $id, {
                    wheel: {
                        selectedIndex: 0
                    }
                });
  
                var that = _this.scroll[$id],
                    active = _this.$('#' + $id + ' .active'),
        ...
        event: function event() {
            ...
            _this.tap(confirm, function () {
                ...
                if (config.confirm) {
                    var flag = config.confirm.call(_this, value);
                    if (flag === false) {
                        return false;
                    } else if (flag) {
                        value = flag;
    ...
    globalThis.RollSelect = RollSelect;
